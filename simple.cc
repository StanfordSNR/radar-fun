...
int UHD_SAFE_MAIN( int argc, char* argv[] )
{
  ...
  std::cout << boost::format( "measured Rx rate: %f MS/s..." ) % ( usrp->get_rx_rate() / 1e6 ) << std::endl
            << std::endl;
  ...
  std::cout << boost::format( "Measured Rx center freq: %f MHz..." ) % ( usrp->get_rx_freq() / 1e6 ) << std::endl
            << std::endl;
  ...
  std::cout << boost::format( "Measured Rx bandwidth: %f MHz..." ) % ( usrp->get_rx_bandwidth() / 1e6 ) << std::endl
            << std::endl;
  ...
  // std::cout << boost::format("Streaming %u samples, for %f seconds...") % total_num_samps % seconds_in_future <<
  // std::endl;
...
std::fill(data.begin(), data.end(), std::complex<double>(0.0,0.0));
  ...
  */
  ...
  while ( not stop_signal_called ) {
    ...
    last_update = now;
    std::cout << boost::format( "Packet #%u got +%u samples at t=%d s over dt=%d ms. Total: %u MS." ) % ct
                   % num_rx_samps % ( tot - seconds_in_future ) % ( 1000 * diff ) % ( num_acc_samps / 1.0e6 )
              << std::endl;
  }
